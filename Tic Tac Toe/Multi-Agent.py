...
def Load():
    ...
    global i, I_AM_ETERNAL, model, HighScore
...
def Test():
    ...
    for temp in range(1_000):
        ...
        if not actions.shape[0] or state.reward == "DONE":
            print('\n' + str(state))
            state = Board()
            PlayersTurn = True
    ...
    for i in range(i, data_size):
        ...
        for epoch in range(lim):
            for temp in range(episodes):
                ...
                while actions.shape[0] and state.reward != "DONE":
                    mean = None
                    action = actions[random.randrange(0, actions.shape[0])]
                    if isModel:
                        mean = QTable.setdefault(repr(state), np.full([9], -1000, dtype = np.float32))
                        index = mean.argmax() if random.randrange(0, 100) < min(95, epoch * 100 // 25) else None
                ...
                it = int(isModel) if state.reward == "DONE" else 2
                X[i][it] += lim / 2 <= epoch
                reward = Y[i][it]
        ...
        if 0.99 * HighScore[0] < X[i][0]:
            ...
            if HighScore[0] < X[i][0] or (HighScore[0] == X[i][0] and HighScore[1] < X[i][1]):
                HighScore = X[i]
                JSON = dict(zip(QTable.keys(), [repr(elem.tolist()) for elem in QTable.values()]))
                json.dump(JSON, open('model.json', 'w'), indent = 4)

        loss = model.fit(X[:None if I_AM_ETERNAL else i + 1], Y[:None if I_AM_ETERNAL else i + 1], epochs = 100, verbose = 0).history['loss'][-1]
